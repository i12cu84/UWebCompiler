...
class UFileOperationsBPLibrary : public UBlueprintFunctionLibrary
{
	...
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Execute Sample function", Keywords = "FileOperations sample test testing"), Category = "FileOperationsTesting")
	...
	{
		/*
		FStringUTF8 ConvertedContent(content);
		bool bWriteSuccess = FFileHelper::SaveStringToFile(ConvertedContent.Get(), *FileName, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
		*/
		bool bWriteSuccess = FFileHelper::SaveStringToFile(content, *FileName, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);

		/*
		std::string ConvertedContent = TCHAR_TO_UTF8(*content);
		bool bWriteSuccess = FFileHelper::SaveStringToFile(FString(ConvertedContent.c_str()), *FileName);
		if (bWriteSuccess)
		{
			return 1;
		}
